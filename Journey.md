...
### Creating the stage 2 part

We want the bootloader to have a "main logic" part (called stage 2).
We will have a CLI, with multiple functions.
We want stage1 to decompress stage2 to a predefined address in RAM, and then jump to it.

But first, let's clean up the code.


### Flash partitions

First, we will make a simple stage 2 code, that will print something to the serial.
Compile and relocate it to some RAM address.
Then we will need to make some "partitions" in our flash file.
Something like:
stage1: 0x0 - 0x1000
stage2: 0x1000 - 0x5000
kernel: 0x5000 - 0x50000

I created the script `format_flash.py` to do just that.

Then we will modify our stage1 code to `memcpy` from the flash (at offset 0x1000) to the ram (at the predefined address stage2 is relocated to).
After copying it, we jump to it.

And voilla!
```
Copying flash from flash to ram
Jumping to stage 2
Hello from stage 2!!
```